...
- **~56 tok/s is the practical ceiling for this kernel structure.** Attention costs ~2.7ms/token (15%, measured by a 3×-dispatch diff) but is NOT improvable by more parallelism: **flash-decoding** (split-KV `attn_flash_partial` over nh·S workgroups + online-softmax `attn_flash_combine`) was built and verified argmax-exact but is **neutral-to-negative at every split** (S=4/8/16 → 52.9/54.9/55.7 vs 56.3) — short decode context (~90 keys) thread-starves each split and the combine dispatch eats the gain; attn's cost is inherent barrier/dispatch latency across 35 sequential layers. Also falsified: **256-thread single-workgroup rmsnorm** (norms look big in `--profile` but are cheap in resident). Reaching the bundle's ~150 needs a **different structure** — its full register-blocked fused `Decode*` pipeline keeping activations in registers across norm+matmul+norm+residual, not incremental kernel tuning. QAT wins on **memory** (2.46GB vs 9.8GB) regardless.
