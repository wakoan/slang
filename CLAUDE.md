...
- Perf: **32 tok/s resident — but this is ~10x below what's achievable, a kernel inefficiency, NOT a fundamental limit.** The model reads only **0.82GB/token** (the 1.17GB PLE table is row-gathered, not read whole), so the bandwidth ceiling is **~334 tok/s** (the browser bundle's ~300 tok/s is near-optimal decode). Our `matvec_dq4`/`matvec_dq2` achieve only ~29 GB/s (162µs for a 4.7MB 2-bit gate) vs the f16 `matvec_wg_packed_v4`'s 235 GB/s — same wg-per-row structure, so the 8x gap is the **f32 dequant compute** (per weight: bit-extract → int→float → f32 FMA), which costs ~8x the weight read. dq2's 16-iteration variable-shift inner loop is especially bad.
- **Diagnosis (measured):** decode is GPU-bound ~20ms/token but achieves only ~41 GB/s (15% of bandwidth) — it's **dispatch/latency-bound, not matmul-compute-bound**. ~680 tiny *dependent* dispatches/token × ~8.5µs launch overhead ≈ 6ms/token of pure overhead, plus activations round-tripping through VRAM between kernels. WON: unrolling `matvec_dq2`'s variable-shift loop (2.5x on that kernel) → **32→41.6 tok/s**. FALSIFIED — all gave ~0 end-to-end despite promising microbenchmarks (back-to-back-dispatch microbenchmarks are unreliable here; trust only end-to-end): subgroup matmul, shared-memory-x tiled matmul, `dot4I8Packed` (the sub-byte→int8 unpack negates the 4-MAC/instr win), one-thread-per-row, vec4 dequant. Tuning individual matmul kernels does not move the wall.
- **The real fix (what the bundle does, unbuilt):** aggressive kernel **fusion** — its decode kernels (`DecodeGateUpNorm`, `DecodeQkvProj`, `DecodeQkNormRope`, `DecodeOprojNorm`, `DecodeDownNormAdd`, `DecodeNormAddNorm`, `DecodePleGate/ProjNorm`) combine norm+matmul+norm+residual into single passes, keeping activations in registers. That cuts dispatch count ~680→~200 and eliminates VRAM round-trips. A large rewrite (~15 fused kernels), est. ~80-120 tok/s. QAT wins on **memory** (2.46GB vs 9.8GB) regardless.
