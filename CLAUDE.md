...
`python -m gemma3.server` (port 8000) serves a WebGPU page that runs the full model in the browser: `web/app.js` mirrors runner.py (CPU-param prefill + GPU-resident chunked decode), `/kernels.json` is generated live from the DSL, and `gemma3/export_web.py` packs weights into `models/.../web/weights.bin` + manifest (GPU layout, f16-packed u32). Browser kernels are the portable set only — packed-u32 via core `unpack2x16float`, barrier-tree reductions, no `shader-f16`/subgroups features. Tokenize/detokenize stay server-side. `tests/test_web_server.py::TestBrowserArtifactsEndToEnd` drives the exported artifacts through wgpu-py with app.js's exact dispatch sequence.
