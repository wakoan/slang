...
- **The fix (what the bundle does, unbuilt):** int8 activation quantization (SRQ — scales are in the checkpoint's `input/output_activation_scale`, currently skipped) + hardware integer dot product `dot4I8Packed` (confirmed accepted by naga) so the matmul is compute-cheap and becomes bandwidth-bound → approaches 300 tok/s. This is a new integer-matmul kernel path, not a tweak. Falsified: vec4 dequant (dynamic `wv[c]` indexing spills on Apple GPUs — 27 vs 32). QAT also still wins on **memory** (2.46GB vs 9.8GB) regardless.
