...
- **Remaining leads toward the bundle's ~150** (each a bigger rewrite): `attn` (~12%) runs only **8 workgroups (one per head)** and, with num_kv_heads=1, all 8 heads **re-read the same KV cache** — output-block the heads (read each K/V row once for all 8). `mv_gateup` (~23%) is maxed at blk8 by the array cap; a **subgroup-reduction** matvec would use one shared array regardless of N and unlock higher blocking. QAT wins on **memory** (2.46GB vs 9.8GB) regardless.
