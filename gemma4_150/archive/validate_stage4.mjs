...
const CHROME="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome", PORT=9253;
const REF="/Users/wako/projects/slang/reference/webml_gemma4_kernels";
const DATA="/private/tmp/claude-501/-Users-wako-projects-slang/ce5a80c9-3495-4ae0-8cd2-813d9c5d619f/scratchpad/stage4.json";
const k73=readFileSync(`${REF}/73_sg_sum.wgsl`,"utf8"); const D=JSON.parse(readFileSync(DATA,"utf8"));
const proc=spawn(CHROME,["--headless=new",`--remote-debugging-port=${PORT}`,"--user-data-dir=/tmp/cdp-s4","--enable-unsafe-webgpu","--use-angle=metal","--enable-features=WebGPU","--no-first-run","--disable-dev-shm-usage","http://localhost:8000/manifest.json"],{stdio:["ignore","ignore","ignore"]});
let ws; const done=o=>{console.log("STAGE4:",JSON.stringify(o));try{ws?.close();}catch{}proc.kill("SIGKILL");process.exit(0);};
try{
 ...
 const expr=`(async()=>{const {k73,D}=${JSON.stringify({k73,D})};
  ...
  const m=dev.createShaderModule({code:k73}); const ci=await m.getCompilationInfo(); const ce=ci.messages.filter(x=>x.type==='error'); if(ce.length)return{compileErrors:ce.map(e=>e.message)};
  ...
  const OUT_F=D.OUT_F;
  const a=mk(b64(D.a)), bits=mk(b64(D.bits)), scale=mk(b64(D.scale)), w12=mk(b64(D.w12));
  ...
  const y2=dev.createBuffer({size:OUT_F*2,usage:ST|DST|SRC}); dev.queue.writeBuffer(y2,0,new Uint16Array(OUT_F));
  const sum2=dev.createBuffer({size:4,usage:ST|DST|SRC}); dev.queue.writeBuffer(sum2,0,new Float32Array(1));
  const par=dev.createBuffer({size:16,usage:UNI|DST}); dev.queue.writeBuffer(par,0,new Float32Array([D.outScale,D.inScale2,0,0]));
  const pipe=dev.createComputePipeline({layout:'auto',compute:{module:m,entryPoint:'main'}});
  const bg=dev.createBindGroup({layout:pipe.getBindGroupLayout(0),entries:[a,bits,scale,pp,hidden,w12,y2,sum2,par].map((b,i)=>({binding:i,resource:{buffer:b}}))});
  const enc=dev.createCommandEncoder();const ps=enc.beginComputePass();ps.setPipeline(pipe);ps.setBindGroup(0,bg);ps.dispatchWorkgroups(192,1,1);ps.end();dev.queue.submit([enc.finish()]);
  ...
  const rbY=dev.createBuffer({size:OUT_F*2,usage:GPUBufferUsage.MAP_READ|DST});
  const rbS=dev.createBuffer({size:4,usage:GPUBufferUsage.MAP_READ|DST});
  const e2=dev.createCommandEncoder();e2.copyBufferToBuffer(hidden,0,rbH,0,OUT_F*4);e2.copyBufferToBuffer(y2,0,rbY,0,OUT_F*2);e2.copyBufferToBuffer(sum2,0,rbS,0,4);dev.queue.submit([e2.finish()]);
  await Promise.all([rbH.mapAsync(1),rbY.mapAsync(1),rbS.mapAsync(1)]);
  const gH=new Float32Array(rbH.getMappedRange()).slice();
  const gY=new Uint16Array(rbY.getMappedRange()).slice();
  const gS=new Float32Array(rbS.getMappedRange())[0];
  const refH=new Float32Array(b64(D.ref_hidden)); const refY=new Uint16Array(b64(D.ref_y2));
  let mxH=0,miH=0; for(let i=0;i<OUT_F;i++){const e=Math.abs(gH[i]-refH[i]);if(e>mxH){mxH=e;miH=i;}}
  let ybad=0,firstBad=-1; for(let i=0;i<OUT_F;i++){if(gY[i]!==refY[i]){ybad++;if(firstBad<0)firstBad=i;}}
  return {hidden_maxAbsDiff:+mxH.toFixed(6), at:miH, gpuH:+gH[miH].toFixed(4), refH:+refH[miH].toFixed(4),
          y2_mismatches:ybad, y2_total:OUT_F, firstBad, sum2_gpu:+gS.toFixed(3), sum2_ref:+D.ref_sum2.toFixed(3)};
 })()`;
