...
// rounding.
var<workgroup> k_tile: array<vec4<f16>, TILE_K * (HEAD_DIM / 4u)>;
var<workgroup> v_tile: array<vec4<f16>, TILE_K * (HEAD_DIM / 4u)>;
...
fn main(
  ...
  loop {
    ...
    for (var i: u32 = tid; i < TILE_K * (HEAD_DIM / 4u); i = i + WG) {
      ...
      if (kj < wgEnd) {
        k_tile[i] = vec4<f16>(k[base4]);
        v_tile[i] = vec4<f16>(v[base4]);
      } else {
