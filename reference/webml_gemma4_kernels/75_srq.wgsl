  ...
  loop {
    ...
    sumA = sumA + (av0.x + av0.y + av0.z + av0.w) + (av1.x + av1.y + av1.z + av1.w);
    {
      ...
      if (o < OUT_F) {
        ...
        q0 = q0 + (dot(vec4<f32>(lo.x, hi.x, lo.y, hi.y), av0)
    ...
    }
    {
      ...
      if (o < OUT_F) {
        ...
        let lo = unpack4x8unorm(p & 0x0F0F0F0Fu);
        let hi = unpack4x8unorm((p >> 4u) & 0x0F0F0F0Fu);
        q1 = q1 + (dot(vec4<f32>(lo.x, hi.x, lo.y, hi.y), av0)
                             + dot(vec4<f32>(lo.z, hi.z, lo.w, hi.w), av1));
      }
    }
    {
      ...
      if (o < OUT_F) {
        ...
        let lo = unpack4x8unorm(p & 0x0F0F0F0Fu);
        let hi = unpack4x8unorm((p >> 4u) & 0x0F0F0F0Fu);
        q2 = q2 + (dot(vec4<f32>(lo.x, hi.x, lo.y, hi.y), av0)
                             + dot(vec4<f32>(lo.z, hi.z, lo.w, hi.w), av1));
      }
    }
    {
      ...
      if (o < OUT_F) {
        ...
        let lo = unpack4x8unorm(p & 0x0F0F0F0Fu);
        let hi = unpack4x8unorm((p >> 4u) & 0x0F0F0F0Fu);
        q3 = q3 + (dot(vec4<f32>(lo.x, hi.x, lo.y, hi.y), av0)
                             + dot(vec4<f32>(lo.z, hi.z, lo.w, hi.w), av1));
      }
    }
    w = w + WG;
